...
# V7 update: MarketCheck fuel_type + hybrid filtering
#
# Run locally:
...
def build_marketcheck_params(
    ...
    # MarketCheck supports the fuel_type vehicle-spec filter.
    # For hybrids, feeds may say "Hybrid" or "Electric / Unleaded", so send both.
    # We still apply a local post-filter because dealer feeds are not always consistent.
    if fuel_filter and fuel_filter.lower() != "any":
        fuel_lookup = {
            "hybrid": "Hybrid,Electric / Unleaded",
...
def fuel_search_text_from_record(record: dict[str, Any]) -> str:
    """Build searchable text for fuel filtering from all likely hybrid indicators."""
    parts = [
