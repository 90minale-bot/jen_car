...
def marketcheck_search(vehicle: VehicleConfig, zip_code: str, radius: int, rows: int) -> List[Dict[str, Any]]:
...
def render_vehicle_section(vehicle: VehicleConfig, zip_code: str, radius: int, rows: int) -> pd.DataFrame:
    ...
    try:
        raw = marketcheck_search(vehicle, zip_code, radius, rows)
...
def main() -> None:
    ...
    st.title("Sister SUV Search: Reliable, Good Gas Mileage — v2")
    st.write(
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.caption("Scoring: price vs market value, miles, one-owner, clean title, distance, AWD, MPG, and useful trims. Range defaults to 50 miles and maxes at 100.")

    all_results: List[pd.DataFrame] = []

    for vehicle in VEHICLES:
        with st.container(border=True):
            df = render_vehicle_section(vehicle, zip_code, radius, rows)
            if not df.empty:
