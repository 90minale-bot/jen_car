...
def marketcheck_search(vehicle: VehicleConfig, zip_code: str, radius: int, rows: int, max_price: int, max_miles: int) -> List[Dict[str, Any]]:
    ...
    params: Dict[str, Any] = {
        ...
        "zip": zip_code,
        "radius": safe_radius,
        "year_min": vehicle.min_year,
        "year_max": vehicle.max_year,
    ...
    for item in listings:
        ...
        if year is None or vehicle.min_year <= int(year) <= vehicle.max_year:
            cleaned.append(item)
...
def render_vehicle_section(vehicle: VehicleConfig, zip_code: str, radius: int, rows: int, max_price: int, max_miles: int) -> pd.DataFrame:
    ...
    try:
        raw = marketcheck_search(vehicle, zip_code, radius, rows, max_price, max_miles)
...
def main() -> None:
    ...
    st.title("Sister SUV Search: Reliable, Good Gas Mileage — v6")
    st.write(
        "This page searches your SUV shortlist and ranks the best used listings by value, miles, ownership/title signals, distance, AWD, and MPG. v6 fixes the $0 Price Gap issue by avoiding MSRP/list-price fallback as market value and using comparable medians when true FMV is missing."
    )
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.caption("Scoring: price vs market value, miles, one-owner, clean title, distance, AWD, MPG, and useful trims. Range defaults to 50 miles and maxes at 100. Cost defaults to $26k and mileage defaults to 40k max.")

    all_results: List[pd.DataFrame] = []

    for vehicle in VEHICLES:
        with st.container(border=True):
            df = render_vehicle_section(vehicle, zip_code, radius, rows, max_price, max_miles)
            if not df.empty:
