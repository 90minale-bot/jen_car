# jens_car_dashboard_web.py
# Jen's Car Dashboard - Web Version with MarketCheck Search
#
# Run locally:
#   streamlit run jens_car_dashboard_web.py
#
...
st.set_page_config(
    page_title="Jen's Car Dashboard",
    page_icon="🚗",
    layout="wide",
)
...
def marketcheck_search(params: dict[str, Any], max_pages: int = 5) -> tuple[list[dict[str, Any]], dict[str, Any]]:
...
def normalize_uploaded_csv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "title" not in df.columns:
        for col in ["make", "model", "trim", "dealer", "city", "state", "vin", "url", "drivetrain", "fuel_type"]:
    ...
    with st.sidebar:
        ...
        if "fuel_type" in filtered.columns:
            hybrid_only = st.checkbox(
                "Hybrid only",
                value=False,
                help="Shows listings where fuel type or title includes hybrid, plug-in hybrid, PHEV, or HEV.",
            )
            if hybrid_only:
                fuel_text = filtered["fuel_type"].fillna("").astype(str).str.lower()
                title_text = filtered["title"].fillna("").astype(str).str.lower() if "title" in filtered.columns else ""
                filtered = filtered[
                    fuel_text.str.contains("hybrid|phev|hev|plug-in", regex=True)
                    | title_text.str.contains("hybrid|phev|hev|plug-in", regex=True)
                ]
...
def main() -> None:
    st.title("🚗 Jen's Car Dashboard")
    st.caption("Search MarketCheck directly, filter cars, and download ranked results.")
    ...
    with st.sidebar:
        ...
        rows_per_page = st.number_input("Rows per page", min_value=10, max_value=50, value=50, step=10)
        max_pages = st.number_input("Max pages", min_value=1, max_value=20, value=4, step=1)
        car_type = st.selectbox("Car type", ["used", "certified", "new", "any"], index=0)
        fuel_filter = st.selectbox(
            "Fuel type",
            ["Any", "Hybrid", "Gasoline", "Diesel", "Electric"],
            index=0,
            help="Hybrid is enforced locally after results return.",
        )

        require_awd = st.checkbox("AWD / 4WD only", value=False)

        search_clicked = st.button("Search cars", type="primary")
        ...
        if not api_key:
            ...
            with st.spinner("Searching MarketCheck..."):
                try:
                    ...
                    if fuel_filter != "Any" and not df.empty:
                        if "fuel_type" not in df.columns:
                            df["fuel_type"] = ""

                        fuel_text = df["fuel_type"].fillna("").astype(str).str.lower()
                        title_text = df["title"].fillna("").astype(str).str.lower() if "title" in df.columns else ""

                        if fuel_filter == "Hybrid":
                            df = df[
                                fuel_text.str.contains("hybrid|phev|hev|plug-in", regex=True)
                                | title_text.str.contains("hybrid|phev|hev|plug-in", regex=True)
                            ]
                            ...
                            df = df[
                                fuel_text.str.contains(fuel_filter.lower(), regex=False)
                                | title_text.str.contains(fuel_filter.lower(), regex=False)
                            ]
